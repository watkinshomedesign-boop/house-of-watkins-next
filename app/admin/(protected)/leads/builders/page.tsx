...
export default async function AdminLeadBuildersPage(props: { searchParams: Promise<SearchParams> }) {
  ...
  if (error) {
    return (
      <div className="mx-auto w-full max-w-6xl">
        <h1 className="text-2xl font-semibold">Lead Signups — Builders</h1>
  ...
  return (
    <div className="mx-auto w-full max-w-6xl">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Lead Signups — Builders</h1>
          <p className="mt-1 text-sm text-neutral-600">Licensed Builder leads.</p>
        </div>
        <div className="flex items-center gap-3">
          <Link
            href={`/admin/leads/homeowners${q ? `?q=${encodeURIComponent(q)}` : ""}`}
          ...
          <Link
            href={`/admin/leads/builders/export${q ? `?q=${encodeURIComponent(q)}` : ""}`}
            className="inline-flex items-center justify-center rounded-lg border border-neutral-200 bg-white px-3 py-2 text-sm font-semibold"
          >
            Export CSV
          </Link>
      ...
      <form className="mt-6" action="/admin/leads/builders" method="get">
      ...
      <div className="mt-4 flex items-center justify-between text-sm text-neutral-600">
        ...
        <div className="flex items-center gap-2">
          <Link
            ...
            href={`/admin/leads/builders?page=${page - 1}${q ? `&q=${encodeURIComponent(q)}` : ""}`}
          >
            Prev
          </Link>
          <Link
