...
export default async function AdminContactsPage(props: { searchParams: Promise<SearchParams> }) {
  ...
  let query = supabase
    ...
    .select("id,created_at,first_name,last_name,email,phone,message,page_path,source", { count: "exact" })
  ...
  if (error) {
    return (
      <div className="mx-auto w-full max-w-5xl">
        ...
        <p className="mt-4 text-sm text-neutral-600">Failed to load contact messages.</p>
  ...
  return (
    <div className="mx-auto w-full max-w-5xl">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Contacts</h1>
          <p className="mt-1 text-sm text-neutral-600">Submissions from the contact page.</p>
        </div>
        <div className="flex items-center gap-3">
          <Link
            href={`/admin/contacts/export${q ? `?q=${encodeURIComponent(q)}` : ""}`}
      ...
      <form className="mt-6" action="/admin/contacts" method="get">
            ...
            {(data ?? []).map((row: any) => (
              <tr key={row.id} className="border-t border-neutral-100 align-top">
                ...
                <td className="px-4 py-3 max-w-[520px]">
                  <div className="whitespace-pre-wrap">{row.message}</div>
                </td>
              </tr>
            ))}
          </tbody>
